 ...
 * 코드개선 사항 항목 by crong 
 * 1. 218라인 if중첩
 * 2. for in ==> forEach를 변경
 * 3. Interval함수 중복기능 합치기
 * 4. 변수, 함수명 변경
 * %. 전역변수로 함수의 기능을 밖으로 빼는 방법
 */
...
// function appendAddListLine(newNode){
// 	var temp = document.querySelector("ol");
// 	var tempLastChild = temp.lastElementChild;
// 	tempLastChild.appendChild(newNode)
// }

function checkRemoveValue(todoORnumber){
	var temp = document.querySelector("ol");
...
function changeAddbox(){
	var temp = document.querySelector("input:nth-child(1)");
	var checkInterval = setInterval(timeInterval,500)
...
function changeRemovebox(){
	var temp = document.querySelector(".removeWrap input");
	var checkInterval = setInterval(timeRemoveInterval,500)
	var checkTime = setTimeout(function(){
		clearInterval(checkInterval);}
		,5000);
}
...
function doAddList(todoORnumber){
	//같은 문자값이 있는지 확인
 	...
 	if(chkValue !== -1){
	 	...
	 	// var newNode2 = newElement(arg2);
	 	// newNode2.innerText = "x"
	 	appendAddList(newNode);
	 	// appendAddListLine(newNode2);
	}
}

function doRemoveList(todoORnumber){
	if(checkRemoveValue(todoORnumber)){
...
function addButtonX(){
	...
	for(i=0; i < temp_list.length; i++){
		temp_list[i].insertAdjacentHTML('beforeend','<BUTTON class="buttonx">x</BUTTON>');
	}
}
